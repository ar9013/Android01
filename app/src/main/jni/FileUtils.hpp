...
namespace FileUtils {
	...
	*/
	...
	void loadKeypointsFromZippedYaml(const std::string& filename,
		std::vector<cv::KeyPoint>& keypoints);
	...
	*/
	...
	void loadStatsFromZippedYaml(const std::string& filename, MatStatsExt& stats);

	void saveStatsToZippedYaml(const std::string& filename, MatStatsExt& stats);
} // namespace FileUtils
